...
/**
* I verify and sign in a user
*/
	struct function signinUser(required struct properties) {
		param name='arguments.properties.emailAddress' default='';
		...
		var result = variables.Validator.validate(theObject=User, context='signin');

		User = variables.UserGateway.getUserByEmailAddress(User);

		User.setPassword(arguments.properties.password);
		...
		if (User.isPersisted()) {
			...
			var failure = {propertyName='emailAddress', clientFieldname='emailAddress', message=message};
	...
	void function setCurrentUser(required any User) {
		getCurrentStorage()[variables.userkey] = arguments.User.getID();
	}
}